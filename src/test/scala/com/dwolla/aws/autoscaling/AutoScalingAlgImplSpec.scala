...
class AutoScalingAlgImplSpec extends Specification with ScalaCheck with IOMatchers with IOImplicits {
  ...
  "AutoScalingAlgImpl" should {
    ...
    "pause 5 seconds and then send a message to restart the lambda" >> {
      val context = cats.effect.laws.util.TestContext()

      val cut = new AutoScalingAlgImpl[IO](null, null)(Async[IO], context.timer[IO])

      for {
        fiber <- cut.pauseAndRecurse.attempt.start(context.contextShift[IO])
        _ <- IO(context.tick(5.seconds))
        result <- fiber.join
      } yield {
        result must beRight[Unit]
      }
    }.pendingUntilFixed("implement sending message to restart lambda")
  }
}
